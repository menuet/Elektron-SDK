...
package com.thomsonreuters.ema.examples.training.niprovider.series300.example320__MarketPrice__Streaming__StatusClosed;
...
public class NiProvider {
	...
	public static void main(String[] args)
	{
		...
		try
		{
          ...
          map.add( EmaFactory.createMapEntry().keyAscii("NI_PUB", MapEntry.MapAction.ADD, filterList));
          ...
          provider.submit( refreshMsg.domainType(EmaRdm.MMT_DIRECTORY).filter( EmaRdm.SERVICE_INFO_FILTER | EmaRdm.SERVICE_STATE_FILTER).payload(map)
			...
			provider.submit( EmaFactory.createStatusMsg().serviceName("NI_PUB").name("IBM.N").state(OmmState.StreamState.CLOSED, OmmState.DataState.SUSPECT,
					OmmState.StatusCode.NONE, "Stream Closed"), itemHandle);
			Thread.sleep(20000);			
		} 
