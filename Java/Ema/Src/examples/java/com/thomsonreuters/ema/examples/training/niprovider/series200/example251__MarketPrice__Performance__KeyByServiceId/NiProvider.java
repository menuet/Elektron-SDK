...
package com.thomsonreuters.ema.examples.training.niprovider.series200.example251__MarketPrice__Performance__KeyByServiceId;
		...
		try
		{
			...
			while (start + 300000 > end)
			{
				for (int handle = 0; handle < itemNumber; ++handle)
				{
					...
					provider.submit( EmaFactory.createUpdateMsg().serviceId(serviceId).name("RTR" + handle + ".N").payload( fieldList ), handle );
					++updateCount;
				}
