	...
	_bIsStreamIdZeroRefreshSubmitted( false )
{
	...
	_handleToStreamId.rehash( _activeConfig.itemCountHint );
...
void OmmNiProviderImpl::removeItems()
{
	...
	_handleToStreamId.clear();

	for ( UInt32 idx = 0; idx < _serviceNameList.size(); ++idx )
	{
		if ( _serviceNameList[idx] ) delete _serviceNameList[idx];
	}
}
...
void OmmNiProviderImpl::readCustomConfig( EmaConfigImpl* pConfigImpl )
{
					...
					else
					{
						...
						for ( UInt32 idx = 0; idx < entryNodeList.size(); ++idx )
						{
							...
							else
							{
								...
								{
									{ "Timeliness::RealTimeEnum", 0 },
									{ "Timeliness::InexactDelayedEnum", 0xFFFFFFFF },
								};
	...
	{
		// todo ...  programmatic configuration needs directory group impl
...
Int64 OmmNiProviderImpl::dispatch( Int64 timeOut )
{
	if ( _activeConfig.operationModel == OmmNiProviderConfig::UserDispatchEnum && !_atExit )
		return rsslReactorDispatchLoop( timeOut, _activeConfig.maxDispatchCountUserThread ) ? OmmProvider::DispatchedEnum : OmmProvider::TimeoutEnum;

	return OmmProvider::TimeoutEnum;
}

UInt64 OmmNiProviderImpl::registerClient( const ReqMsg& reqMsg, OmmProviderClient& ommConsClient, void* closure, UInt64 parentHandle )
{
	OmmClient< OmmNiProviderClient >* pClient = 0;

	try
	{
		pClient = new OmmClient< OmmNiProviderClient >( static_cast<OmmNiProviderClient*>( &ommConsClient ) );
	}
	catch ( std::bad_alloc ) {}

	if ( !pClient )
	{
		handleMee( "Failed to allocate memory in OmmNiProviderImpl::registerClient( const ReqMsg& )" );
		return 0;
	}

	_userLock.lock();

	UInt64 handle = _pItemCallbackClient ? _pItemCallbackClient->registerClient( reqMsg, pClient, closure, parentHandle ) : 0;
...
void OmmNiProviderImpl::submit( const RefreshMsg& msg, UInt64 handle )
{
	...
	if ( submitMsgOpts.pRsslMsg->msgBase.domainType == ema::rdm::MMT_DIRECTORY )
	{
		...
		{
			Int32* pStreamId = _handleToStreamId.find( handle );

			if ( pStreamId )
			{
				submitMsgOpts.pRsslMsg->msgBase.streamId = *pStreamId;
			}
			else
			{
				submitMsgOpts.pRsslMsg->msgBase.streamId = -pChannel->getNextStreamId();

				_handleToStreamId.insert( handle, submitMsgOpts.pRsslMsg->msgBase.streamId );
	...
	if ( rsslReactorSubmitMsg( pChannel->getRsslReactor(), pChannel->getRsslChannel(), &submitMsgOpts, &rsslErrorInfo ) != RSSL_RET_SUCCESS )
	{
		if ( bHandleAdded )
		{
			_handleToStreamId.erase( handle );
