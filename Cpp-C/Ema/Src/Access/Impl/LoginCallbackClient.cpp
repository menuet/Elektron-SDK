...
bool Login::populate( RsslStatusMsg& status, RsslBuffer& buffer )
{
	...
	status.msgBase.msgKey.flags |= RSSL_MKF_HAS_ATTRIB | RSSL_MKF_HAS_NAME | RSSL_MKF_HAS_NAME_TYPE;
	...
	status.msgBase.msgKey.attribContainerType = RSSL_DT_ELEMENT_LIST;
	status.msgBase.msgKey.encAttrib = buffer;
...
void LoginCallbackClient::processChannelEvent( RsslReactorChannelEvent* pEvent )
{
	...
	case RSSL_RC_CET_CHANNEL_UP :
	{
		...
		for ( UInt32 idx = 0; idx < _loginItems.size(); ++idx )
		{
			LoginItem* item = _loginItems[idx];
			item->getClientFunctions()->onAllMsg( _statusMsg, item );
			item->getClientFunctions()->onStatusMsg( _statusMsg, item );
		}
	}
...
LoginItem* LoginCallbackClient::getLoginItem( const ReqMsg&, ClientFunctions* clientFunctions, void* closure )
{
	LoginItem* li = LoginItem::create( _ommBaseImpl, clientFunctions, closure, _loginList );
...
LoginItem* LoginItem::create( OmmBaseImpl& ommBaseImpl, ClientFunctions* clientFunctions, void* closure, const LoginList& loginList )
{
	LoginItem* pItem = 0;

	try
	{
		pItem = new LoginItem( ommBaseImpl, clientFunctions, closure, loginList );
	}
	catch ( std::bad_alloc ) {}
...
LoginItem::LoginItem( OmmBaseImpl& ommBaseImpl, ClientFunctions* clientFunctions, void* closure, const LoginList& loginList ) :
...
bool LoginItem::open( RsslRDMLoginRequest* rsslRdmLoginRequest, const LoginList& loginList )
{
	...
	_ommBaseImpl.setDispatchInternalMsg();
